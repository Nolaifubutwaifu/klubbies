    ...
    <>
      <header className="flex flex-wrap items-center justify-between gap-4 border-b-2 border-divider px-6 py-4">
        <div className="flex flex-wrap items-center gap-3">
          ...
          {adminArea ? <span className="tag tag-accent text-[11px]">Admin view</span> : null}
        </div>
        <div className="flex flex-wrap items-center gap-3">
          ...
          {perms.manage_club ? (
            ...
            <span className="tag tag-outline text-[11px]">{ctx.role?.name ?? "Member"}</span>
          )}
      ...
      {adminArea ? (
        ...
        <nav className="flex flex-wrap gap-1 border-b-2 border-divider px-6 py-2">
          {memberLinks.map((link) => (
            <Link key={link.href} href={link.href} className="btn btn-ghost text-[13px]">
              {link.label}
            </Link>
          ))}
        </nav>
